...
package org.homeautomation.picavi.plugin.impl.dao.impl;
...
import org.homeautomation.picavi.plugin.impl.util.DeviceTypeUtils;
...
/**
 * Implements IotDeviceDAO for picavi Devices.
 */
public class DeviceTypeDAOImpl {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("picavi device " + deviceId + " data has been fetched from " +
                            "picavi database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while fetching picavi device : '" + deviceId + "'";
            log.error(msg, e);
    ...
    public boolean addDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("picavi device " + device.getDeviceIdentifier() + " data has been" +
                            " added to the picavi database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while adding the picavi device '" +
                    device.getDeviceIdentifier() + "' to the picavi db.";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
    ...
    public boolean updateDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String msg = "Error occurred while modifying the picavi device '" +
    ...
    public boolean deleteDevice(String deviceId) throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("picavi device " + deviceId + " data has deleted" +
                            " from the picavi database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while deleting picavi device " + deviceId;
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, null);
    ...
    public List<Device> getAllDevices() throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("All picavi device details have fetched from picavi database.");
            }
            return iotDevices;
        } catch (SQLException e) {
            String msg = "Error occurred while fetching all picavi device data'";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, resultSet);
    ...
    public void addEdgeDevice(EdgeDevice edgeDevice) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String msg = "Error occurred while adding the edge device '" +
                         edgeDevice.getEdgeDeviceSerial() + "' to the picavi db.";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, null);
