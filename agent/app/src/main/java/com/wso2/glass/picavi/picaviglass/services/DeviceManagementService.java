...
import com.wso2.glass.picavi.picaviglass.mqtt.AndroidMQTTHandler;
...
public class DeviceManagementService extends Service {
    ...
    private AndroidMQTTHandler androidMQTTHandler;
    ...
    public void onCreate() {
        androidMQTTHandler = new AndroidMQTTHandler(this, new MessageReceivedCallback() {
            @Override
        ...
        androidMQTTHandler.connect();
    }

    @Override
    public void onDestroy() {
        if (androidMQTTHandler != null && androidMQTTHandler.isConnected()) {
            androidMQTTHandler.disconnect();
        }
        androidMQTTHandler = null;
    }
    ...
    private void sendATResponse(String message) {
        try {
            ...
            if (androidMQTTHandler.isConnected()) {
                androidMQTTHandler.publishDeviceData(wrapper.toString());
            }
    ...
    private void publishStats(String topic, String key, float value) {
        try {
            ...
            wrapper.put("event", jsonEvent);
            if (androidMQTTHandler.isConnected()) {
                androidMQTTHandler.publishDeviceData(wrapper.toString(), topic);
            }
        } catch (TransportHandlerException | JSONException e) {
            Log.e(TAG, e.getClass().getSimpleName(), e);
        }
