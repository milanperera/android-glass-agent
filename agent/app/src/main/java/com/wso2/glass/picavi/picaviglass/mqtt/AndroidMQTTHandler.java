 ...
 * under the License.
 */
package com.wso2.glass.picavi.picaviglass.mqtt;
...
import com.wso2.glass.picavi.picaviglass.mqtt.transport.MQTTTransportHandler;
 ...
 */
public class AndroidMQTTHandler extends MQTTTransportHandler {

    private static final String TAG = AndroidMQTTHandler.class.getSimpleName();
    ...
    /**
     * Default constructor for the AndroidMQTTHandler.
     */
    public AndroidMQTTHandler(Context context, MessageReceivedCallback messageReceivedCallback) {
        super(context);
        this.context = context;
        this.messageReceivedCallback = messageReceivedCallback;
        this.publishTopic = LocalRegistry.getTenantDomain(context)+ "/" + Constants.DEVICE_TYPE + "/" +
    ...
    public void connect() {
        Runnable connector = new Runnable() {
            public void run() {
                PicaviClient client = new PicaviClient(context);
                while (!isConnected()) {
                    client.renewToken();
                ...
                while (isConnected()) {
                    try {
                        ...
                        Log.w(TAG, "Unable to 'STOP' MQTT connection at broker at: " + mqttBrokerEndPoint
                                + " for device-type - " + Constants.DEVICE_TYPE, e);
